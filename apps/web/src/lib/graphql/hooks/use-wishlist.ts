...
export function useAddItemLink() {
  ...
  return createMutation(() => ({
    mutationFn: ({ itemId, input }: { itemId: string; input: ItemLinkInput }) =>
...
export function useUpdateItemLink() {
  ...
  return createMutation(() => ({
    mutationFn: ({ id, input }: { id: string; input: ItemLinkUpdateInput }) =>
...
export function useSetPrimaryLink() {
  ...
  return createMutation(() => ({
    ...
    onSuccess: (_: unknown, variables: { itemId: string; linkId: string }) => {
      queryClient.invalidateQueries({ queryKey: ["wishlist"] });
      queryClient.invalidateQueries({
        queryKey: ["wishlist", "item", variables.itemId],
      });
