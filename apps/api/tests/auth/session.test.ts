...
describe("session helpers", () => {
    ...
    it("sets insecure cookies when INSECURE_COOKIES env var is set", () => {
        const ctx = createContext({ dev: false, insecureCookies: true });
        setSessionCookie(ctx, "session-id");
    ...
    it("URL-encodes session IDs with special characters", () => {
        const ctx = createContext({ dev: false });
        const specialSessionId = "abc123;malicious=value";
        setSessionCookie(ctx, specialSessionId);

        const headers = ctx.getRecordedHeaders();
        expect(headers["Set-Cookie"]).toContain(
            "corvus-session=abc123%3Bmalicious%3Dvalue",
        );
        expect(headers["Set-Cookie"]).toContain("Secure");
    });

    it("clears the session cookie with zero max age and expires", () => {
        const ctx = createContext({ dev: true });
        ...
        expect(headers["Set-Cookie"]).toContain("Expires=0");
    });
